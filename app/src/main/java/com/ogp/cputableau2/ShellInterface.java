...
// uid=0(root) gid=0(root)
	...
	private static String _runCommand (String 	command, 
									   OUTPUT 	o) throws IOException 
	{
		...
		try 
		{
			...
			InputStreamHandler sh = sinkProcessOutput (process, 
													   o);
	...
	public static InputStreamHandler sinkProcessOutput (Process 	p, 
														OUTPUT 		o) 
	{
		...
		{
		case STDOUT:
			output = new InputStreamHandler(p.getErrorStream(), 
											false);
		...
		case STDERR:
			output = new InputStreamHandler(p.getInputStream(), 
											false);
